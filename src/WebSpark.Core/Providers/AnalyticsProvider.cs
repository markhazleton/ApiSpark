...
public class AnalyticsProvider : IAnalyticsProvider
{
    ...
    public async Task<Models.AnalyticsModel> GetAnalytics()
    {
        ...
        var model = new Models.AnalyticsModel()
        {
            ...
            LatestPostViews = GetLatestPostViews() ?? new Models.BarChartModel(),
    ...
    {
        var blog = await _db.Blogs.OrderBy(b => b.Id).FirstOrDefaultAsync();
        if (blog == null) return false;
    ...
    {
        var blog = _db.Blogs.OrderBy(b => b.Id).FirstOrDefault();
        var period = blog?.AnalyticsPeriod == 0 ? 3 : blog?.AnalyticsPeriod ?? 3;
        ...
        {
            Labels = [.. posts.Select(p => p.Title)],
            Data = [.. posts.Select(p => p.PostViews)]
        };
    }
