    ...
    : IMenuProvider, IDisposable, IMenuService
{
    ...
    public async Task<MenuEditModel> GetMenuEditAsync(int Id)
    {
        ...
        returnMenu.Parents = [.. Create([.. menuList.Where(w => w.Parent == null)]).Select(s => new Models.LookupModel() { Value = s.Id.ToString(), Text = s.Title })];
        returnMenu.Parents.Insert(0, new Models.LookupModel() { Value = string.Empty, Text = "None" });

        returnMenu.Domains = [.. (await webDomainContext.Set<WebSite>().ToListAsync()).Select(s => new Models.LookupModel() { Value = s.Id.ToString(), Text = s.Name })];
        return returnMenu;
    }
